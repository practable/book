...
`/patch` does not compile (deliberately) - it is code that is used by `./scripts/generate_client.sh` to fix autogenerated swagger code for some nicer printing of error messages in the console client

Therefore running `go test ./...` in the root dir fails, because patch won't compile

Instead run `go test ./internal/...` and `go test ./pkg/...`
