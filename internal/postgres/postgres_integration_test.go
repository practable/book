...
func integrationRepository(t *testing.T) *Repository {
	...
	lock, err := pgx.Connect(context.Background(), url)
	require.NoError(t, err)
	_, err = lock.Exec(context.Background(), "SELECT pg_advisory_lock(1651470187)")
	require.NoError(t, err)
	t.Cleanup(func() {
...
func TestManualHealthCheckIsAtomicIdempotentAndAutoCloses(t *testing.T) {
	...
	release, err := repository.RequestVerifiedResourceRelease(ctx, "resource-a", []string{"video"}, "technician", 0, now.Add(-time.Second))
	require.NoError(t, err)
...
func TestInteractiveMaintenanceActivationRunsWhileResourceHeld(t *testing.T) {
	repository := integrationRepository(t)
	ctx := context.Background()
	now := time.Date(2026, 9, 1, 10, 0, 0, 0, time.UTC)
	bookingRequest := request("interactive-maintenance", "technician", "slot-a", "resource-a", now.Add(-time.Minute), now.Add(time.Hour))
	bookingRequest.Maintenance = true
	bookingRequest.Booking.Maintenance = true
	bookingRequest.Booking.Policy = "__maintenance__"
	_, _, err := repository.CreateBooking(ctx, bookingRequest)
	require.NoError(t, err)
	require.NoError(t, repository.SetResourceAvailabilityBy(ctx, "resource-a", false, "technician maintenance", "technician", 0))
	req := activationRequest(bookingRequest.Booking.Name, bookingRequest.Booking.User, "interactive-maintenance", now)
	req.Stages = req.Stages[:1]
	run, _, err := repository.CreateActivation(ctx, req)
	require.NoError(t, err)
	_, _, err = repository.ApplyCallback(ctx, operations.Callback{DeliveryID: "interactive-maintenance-accepted", JobID: run.Stages[0].JobID,
		State: "accepted", At: now}, "interactive-maintenance-accepted-hash")
	require.NoError(t, err)
	_, _, err = repository.ApplyCallback(ctx, operations.Callback{DeliveryID: "interactive-maintenance-succeeded", JobID: run.Stages[0].JobID,
		State: "succeeded", At: now.Add(time.Second)}, "interactive-maintenance-succeeded-hash")
	require.NoError(t, err)
	run, err = repository.GetActivation(ctx, run.ID)
	require.NoError(t, err)
	require.Equal(t, "active", run.State)
	booking, err := repository.GetBooking(ctx, bookingRequest.Booking.Name)
	require.NoError(t, err)
	require.True(t, booking.Booking.Started)
	state, err := repository.Load(ctx)
	require.NoError(t, err)
...
func TestCleanupRunAndAccountingSurviveRepositoryRestart(t *testing.T) {
	...
	ctx := context.Background()
	now := time.Date(2026, 9, 1, 10, 0, 0, 0, time.UTC)
	_, _, err := repository.CreateBooking(ctx, request("restart-cleanup", "opaque-user", "slot-a", "resource-a", now.Add(-time.Minute), now.Add(time.Hour)))
	require.NoError(t, err)
	req := activationRequest("restart-cleanup", "opaque-user", "restart-cleanup", now)
	req.Stages = req.Stages[:1]
	req.CleanupStages = []operations.ActivationStageSpec{{Name: "switch-off", JobTemplate: "video-off", Workflow: "video-off",
		DueAt: now, TimeoutAt: now.Add(4 * time.Second), MaximumAttempts: 1, Parameters: map[string]string{"stream": "camera-a"}, ProgressMessage: "Switching video off"}}
	run, _, err := repository.CreateActivation(ctx, req)
	require.NoError(t, err)
	preparationJob := run.Stages[0].JobID
	_, _, err = repository.ApplyCallback(ctx, operations.Callback{DeliveryID: "restart-preparation-accepted", JobID: preparationJob, State: "accepted", At: now}, "restart-preparation-accepted-hash")
	require.NoError(t, err)
	_, _, err = repository.ActivateOperationalJob(ctx, preparationJob, "restart-preparation-activate", "restart-preparation-activate-hash", now)
	require.NoError(t, err)
	_, _, err = repository.ApplyCallback(ctx, operations.Callback{DeliveryID: "restart-preparation-succeeded", JobID: preparationJob, State: "succeeded", At: now.Add(time.Second)}, "restart-preparation-succeeded-hash")
	require.NoError(t, err)
	_, err = repository.CancelBooking(ctx, req.BookingName, now.Add(2*time.Second), "opaque-user", 2*time.Second, 0)
	require.NoError(t, err)
	run, err = repository.GetActivation(ctx, run.ID)
	require.NoError(t, err)
	require.Equal(t, "running", run.CleanupState)
	...
	repository.Close()
	reopened, err := Open(ctx, databaseURL, 4, 10*time.Second)
	require.NoError(t, err)
	t.Cleanup(reopened.Close)
	run, err = reopened.GetActivation(ctx, run.ID)
	require.NoError(t, err)
	require.Equal(t, "running", run.CleanupState)
	require.Equal(t, cleanupJob, run.CleanupStages[0].JobID)
	...
	require.Equal(t, int64(2), ledgerRows)
}
