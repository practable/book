...
func (r *Repository) GetOperationalUsageSummary(ctx context.Context, query store.UsageQuery) (time.Duration, time.Duration, int64, error) {
	ctx, cancel := context.WithTimeout(ctx, r.operationTimeout)
	defer cancel()
	var preparation, cleanup int64
	var jobs int64
	err := r.pool.QueryRow(ctx, `SELECT
		...
		Scan(&preparation, &cleanup, &jobs)
	return time.Duration(preparation), time.Duration(cleanup), jobs, err
}
