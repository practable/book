...
func (o *AddGroupForUserURL) Build() (*url.URL, error) {
	...
	var _path = "/users/{user_name}/groups/{group_name}"

	groupName := o.GroupName
	if groupName != "" {
		_path = strings.Replace(_path, "{group_name}", groupName, -1)
	} else {
		return nil, errors.New("groupName is required on AddGroupForUserURL")
	}
	...
	if userName != "" {
		...
		return nil, errors.New("userName is required on AddGroupForUserURL")
	}
...
func (o *AddGroupForUserURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on AddGroupForUserURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on AddGroupForUserURL")
	}
