...
func (o *CancelBookingURL) Build() (*url.URL, error) {
	...
	var _path = "/users/{user_name}/bookings/{booking_name}"

	bookingName := o.BookingName
	if bookingName != "" {
		_path = strings.Replace(_path, "{booking_name}", bookingName, -1)
	} else {
		return nil, errors.New("bookingName is required on CancelBookingURL")
	}
	...
	if userName != "" {
		...
		return nil, errors.New("userName is required on CancelBookingURL")
	}
...
func (o *CancelBookingURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on CancelBookingURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on CancelBookingURL")
	}
