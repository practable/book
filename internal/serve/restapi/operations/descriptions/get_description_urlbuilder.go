...
func (o *GetDescriptionURL) Build() (*url.URL, error) {
	...
	var _path = "/descriptions/{description_name}"

	descriptionName := o.DescriptionName
	if descriptionName != "" {
		_path = strings.Replace(_path, "{description_name}", descriptionName, -1)
	} else {
		return nil, errors.New("descriptionName is required on GetDescriptionURL")
	}
...
func (o *GetDescriptionURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetDescriptionURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetDescriptionURL")
	}
