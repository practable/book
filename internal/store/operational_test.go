...
func TestScheduledOperationDefaultsCostOwnerToResource(t *testing.T) {
	m := operationalManifest()
	...
	if owner := operationalCostOwner("tank", m.Resources["tank"].Operations); owner != "tank" {
	...
	if owner := operationalCostOwner("tank", m.Resources["tank"].Operations); owner != "physics-teaching-lab" {
		t.Fatalf("explicit owner = %q", owner)
	}
}
