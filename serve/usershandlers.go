...
import (
	...
	"github.com/timdrysdale/interval"
	lit "github.com/timdrysdale/interval/login"
	"github.com/timdrysdale/interval/serve/models"
	"github.com/timdrysdale/interval/serve/restapi/operations/users"
)

// getAccessTokenHandler
// minlen is the minimum length of the usernames that will be accepted
// dur is the time in seconds for which to issue the booking token
func getAccessTokenHandler(host, secret string, minlen, duration int, s *interval.Store) func(users.GetAccessTokenParams, interface{}) middleware.Responder {
	return func(params users.GetAccessTokenParams, principal interface{}) middleware.Responder {
		...
		if len(params.UserName) < minlen {
			c := "404"
			m := "user name must be " + strconv.Itoa(minlen) + " or more alphanumeric characters"
		...
		later := jwt.NewNumericDate(s.Now().Add(time.Duration(duration) * time.Second))
