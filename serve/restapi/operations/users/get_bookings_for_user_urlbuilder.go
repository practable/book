...
func (o *GetBookingsForUserURL) Build() (*url.URL, error) {
	...
	var _path = "/users/{user_name}/bookings"

	userName := o.UserName
	if userName != "" {
		_path = strings.Replace(_path, "{user_name}", userName, -1)
	} else {
		return nil, errors.New("userName is required on GetBookingsForUserURL")
	}
...
func (o *GetBookingsForUserURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetBookingsForUserURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetBookingsForUserURL")
	}
