...
// Required for cancelling existing sessions
// BookingID (added for booked sessions)

// Token represents a JWT token
type Token struct {

	// Booking ID represents the booking associated with the token
	// permitting tokens from cancelled bookings to be identified, and denied
...
// SetBookingID sets the bookingID for token topic
func SetBookingID(token *Token, bid string) {
	token.BookingID = bid
}
...
// HasRequiredClaimsAndBookingID returns false if the Token is missing any required elements
func HasRequiredClaimsAndBookingID(token Token) bool {
