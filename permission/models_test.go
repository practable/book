...
func TestAddBookingID(t *testing.T) {
	...
	assert.Equal(t, "", token.BookingID)

	assert.True(t, HasRequiredClaims(token))
	assert.False(t, HasRequiredClaimsAndBookingID(token))

	SetBookingID(&token, "abc123")

	assert.Equal(t, "abc123", token.BookingID)
	assert.True(t, HasRequiredClaimsAndBookingID(token))
