...
export BOOKCLIENT_SECRET=somesecret
export BOOKCLIENT_TOKEN_TTL=300
export BOOKCLIENT_TOKEN_ADMIN=true
export BOOKCLIENT_TOKEN_AUD=https://book.example.io
export BOOKCLIENT_TOKEN_SUB=someuser
bearer=$(book token)

If you want to set a future NBF date, then specify the NBF in RFC3339 format
export BOOKCLIENT_TOKEN_NBF=2022-10-12T07:20:50Z
`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if aud == "" {
			fmt.Println("BOOKCLIENT_TOKEN_AUD not set")
			os.Exit(1)
		}
		if sub == "" {
			fmt.Println("BOOKCLIENT_TOKEN_SUB not set")
			os.Exit(1)
		}
		if secret == "" {
			fmt.Println("BOOKCLIENT_SECRET not set")
			os.Exit(1)
		}
		if ttl == "" {
			fmt.Println("BOOKCLIENT_TOKEN_TTL not set")
			os.Exit(1)
		}
		...
		if nbfstr != "" {
			t, e := time.Parse(
				...
				nbfstr)
			if e != nil {
				fmt.Printf("BOOKCLIENT_TOKEN_NBF time format error: %s\n", e.Error())
			}
			// ensure future date
			if t.After(time.Now()) {
				nbf = t.Unix()
		...
		if err != nil {
			fmt.Printf("BOOKCLIENT_TOKEN_TTL duration format error: %s\n", err.Error())
		}
